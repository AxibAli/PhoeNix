...
namespace RO
{
	public class ServerVersionCheckOpt : VersionUpdateOperation
	{
		...
		{
			base.StartCheck ();
			//			_cdnAddress = "http://ro-xdcdn.b0.upaiyun.com/res/" + ApplicationHelper.platformFolder + "/";
			_retryTimes = 0;
			_errorTimes = 0;
			//_cdnAddress = BuildBundleEnvInfo.CdnWithEnv;
			_cdnAddress = "http://103.85.85.216:9797/res/" + ApplicationHelper.platformFolder + "/";
			_manager.ShowUpdate ("", ROWords.CHECKING_REMOTE_VERSION);
			_manager.UpdateProgress (0);
			//加载本地安装版本号
			LoadStreamVersion ();
			_manager.ShowUpdate ("","111111");
			//加载本地缓存版本号
			LoadSaveVersion ();
			_manager.ShowUpdate ("","222222");
			//下载服务器版本
		...
		void TryGetRemoteClientVersion ()
		{
			...
			if (data.IsArray) {
				for (int i = 0; i < data.Count; i++) {
					...
					HttpWWWRequestOrder order = new HttpWWWRequestOrder (url + "/versionios.php?", _timeOut, operation, false, true);
			...
			_manager.ShowYesConfirm (content, ROWords.GOTO, () => {
				...
				if (infos != null && infos.Count > 0) {
					...
					if (string.IsNullOrEmpty (appUrl)) {
						...
						Application.OpenURL ("http://103.85.85.216:9797/?dir=RO" + chinese);
					} else {  
